export const messages = {
  errors: {
    ...
    errorFetchingWeatherData: (error: unknown) =>
    ...
    incompleteDailyDataVariables: (date: string) =>
      `Incomplete daily weather data variables for date: ${date}`,
    failedUploadDataBucket: (bucket: string, key: string) =>
      `Failed to upload data to ${bucket}/${key}`,
    failedUploadData: (error: unknown) =>
      `Failed to upload data to S3: ${error}`,
  },
  success: {
    dataUploaded: (bucket: string, key: string) =>
      `Data successfully uploaded to ${bucket}/${key}`,
